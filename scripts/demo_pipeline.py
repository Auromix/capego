    ...
    try:
        ...
        capture = subprocess.run(
            [
                ...
                "50",
                "--outbox",
                str(args.root / "device" / f"{rid}.sqlite3"),
            ],
            capture_output=True,
            text=True,
            check=True,
        ...
        assert not client.get("/api/v1/processing").json()["items"] or rid not in [
            j["recording_id"] for j in client.get("/api/v1/processing").json()["items"]
        ]
        response = client.post(
            "/api/v1/processing", json={"recording_ids": [rid], "backend": "synthetic"}
        )
        ...
        report = {
            ...
            "origin": "synthetic",
            "recording_id": rid,
            "packets": result["produced"],
            "processing_id": jid,
            "dataset_id": did,
            "exports": exports,
            "receiver_stopped_after_demo": True,
        }
