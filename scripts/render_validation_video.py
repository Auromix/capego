...
def main():
    ...
    clips, cues, sources, elapsed = [], [], [], 0.0
    for index, chapter in enumerate(data["chapters"]):
        ...
        draw.text(
            (1590, 37),
            f"{index + 1:02d} / {len(data['chapters']):02d}",
            font=fonts[23],
            fill="#d7e7de",
        )
        ...
        if chapter.get("image"):
            ...
            sources.append(
                {"path": chapter["image"], "sha256": hashlib.sha256(path.read_bytes()).hexdigest()}
            )
        else:
        ...
        if args.voice:
            ...
            with wave.open(str(audio)) as wav:
                duration = max(duration, wav.getnframes() / wav.getframerate() + 0.8)
