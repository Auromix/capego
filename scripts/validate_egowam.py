...
def main():
    ...
    if subprocess.check_output(
        ["git", "-C", str(upstream), "diff", "--name-only", "HEAD"], text=True
    ).strip():
    ...
    datasets = [
        ZarrDataset(
            args.export / e["path"],
            Human.get_keymap(action_horizon=horizon, use_future=True),
            Human.get_transform_list(chunk_length=horizon, stride=1, use_future=True),
        )
    ...
    if not samples:
        raise ValueError(
            "Need an episode with more than four frames for unpadded future supervision"
        )
    ...
    images, future = images / 255.0, future / 255.0
    ...
    model = HPTModel(
        ...
        num_blocks=1,
        num_heads=2,
        token_postprocessing="joint_token",
    ...
    model.init_domain_stem(
        domain,
        {
            ...
            "state_ee_pose": MLPPolicyStem(
                input_dim=12, output_dim=width, widths=[width], specs=specs
            ),
        },
    )
    ...
    model.auxiliary_loss_weights = {"future_rgb": 1.0}
        ...
        losses.append(
            {
                ...
                "action": float(model._last_loss_breakdown["action"]),
    ...
    changes = {
        ...
        for key, prefix in {
            "trunk": "trunk.",
            "action_head": f"heads.{domain}.",
            "world_head": f"heads.{domain}_future_rgb.",
        }.items()
    }
    ...
    result = {
        ...
        "target": "EgoWAM HPTModel joint world/action loss",
        "scope": "reduced configuration smoke test; not released weights or full training recipe",
        "device": args.device,
        "torch": torch.__version__,
        "source_origins": report["origins"],
        "dataset_sha256": report["dataset_sha256"],
        "batch_size": len(samples),
        "action_shape": list(actions.shape),
        "future_rgb_shape": list(future.shape),
        "losses": losses,
        "updated_parameter_tensors": changes,
        "robot_samples": 0,
