...
def synthetic_spec(recording_id=None, width=320, height=240, fps=10, imu_hz=100, chunk_seconds=1):
    return RecordingSpec(
        id=recording_id or f"rec-{uuid.uuid4().hex}", device_id="synthetic-rig", origin="synthetic",
        started_at=utc_now(), chunk_duration_ns=int(chunk_seconds * 1e9),
        streams=[Stream(id="left_rgb", kind="rgb", rate_hz=fps, width=width, height=height),
                 Stream(id="right_rgb", kind="rgb", rate_hz=fps, width=width, height=height),
                 Stream(id="imu", kind="imu", rate_hz=imu_hz)],
        calibration={"status": "synthetic", "version": "synthetic-v1", "length_unit": "m",
                     "reference_camera": "left_rgb", "baseline_m": 0.06,
                     "camera_axes": "x_right_y_down_z_forward", "note": "Synthetic fixture, not physical calibration"},
    )
...
def synthetic_image(stream: Stream, timestamp_ns: int) -> bytes:
    ...
    draw.rectangle((width * .58, height * .44, width * .88, height * .79), fill="#c8a779", outline="#675d4c", width=3)
    t = timestamp_ns / 1e9
    x = width * (0.3 + .12 * math.sin(t)) + (-6 if stream.id == "right_rgb" else 0)
    y = height * .55
    draw.ellipse((x - width * .06, y - height * .12, x + width * .06, y + height * .12), fill="#dc754f", outline="#833d2f", width=2)
    for hx in [width * .2, x]:
        draw.line((hx, height, hx + width * .05, height * .65), fill="#e1b99c", width=max(3, int(width * .045)))
...
def synthetic_packets(spec: RecordingSpec, duration_seconds: float):
    ...
    while queue:
        ...
        if stream.kind == "rgb":
            ...
            codec = "imu_json"
            payload = canonical({"accel_m_s2": [0., 0., 9.80665], "gyro_rad_s": [0., 0.02 * math.sin(timestamp_ns / 1e9), 0.]})
        yield Packet(recording_id=spec.id, sequence=sequence, stream_id=stream.id,
                     timestamp_ns=timestamp_ns, source_timestamp_ns=timestamp_ns,
                     clock_id="synthetic_clock", codec=codec,
