...
def create_dataset(store, request: DatasetRequest):
    ...
    for selection in request.selections:
        ...
        for segment_id in selection.segment_ids:
            ...
            if key in seen:
                raise StoreError(
                    "duplicate_selection", "Dataset contains a duplicate source segment", 422
                )
            seen.add(key)
            ...
            if decision == "excluded" or (blocked and decision != "usable"):
                raise StoreError(
                    "review_required",
                    "Selected segment is excluded or has unresolved review requirements",
                )
            ...
            if "task_id" not in span:
                ...
                if any(
                    s["review"]["decision"] == "excluded"
                    or (s["needs_review"] and s["review"]["decision"] != "usable")
                    for s in children
                ):
                    raise StoreError(
                        "unresolved_child",
                        "Select usable operations individually while this task has excluded/unresolved operations",
                    )
            for previous in items:
                if previous["recording_id"] == rec["id"] and max(
                    previous["segment"]["start_ns"], span["start_ns"]
                ) < min(previous["segment"]["end_ns"], span["end_ns"]):
                    raise StoreError(
                        "overlapping_selection",
                        "Overlapping source intervals must be exported as separate dataset versions",
                        422,
                    )
            items.append(
                {
                    ...
                    "processing_sha256": result_digest(result),
    ...
    with store.connection(write=True) as db:
        db.execute(
            ...
            (body["id"], body["name"], encoded.decode(), digest, body["created_at"]),
        )
...
def list_datasets(store, limit=100, offset=0):
    with store.connection() as db:
        rows = db.execute(
            "SELECT id,name,sha256,created_at FROM datasets ORDER BY created_at DESC LIMIT ? OFFSET ?",
            (limit, offset),
        ).fetchall()
...
def validate_sources(store, dataset):
    for item in dataset["items"]:
        ...
        if (
            rec["end"]["content_sha256"] != item["source_sha256"]
            or result_digest(result) != item["processing_sha256"]
        ):
