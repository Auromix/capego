...
> 状态：本页记录截至 2026-09-24 逐项讨论确认的产品行为。软件原型实现见[第一版协议](protocol-v1.md)和[工作台与处理](workbench-v1.md)，证据见[验证记录](../validation/2026-09-24-software.md)；实体器件、硬同步和生产性能尚未验证。
...
软件原型采用 SQLite 临时 outbox、顺序重试、内容摘要去重，以及文件 fsync 与数据库事务提交后的保存确认，详见[第一版协议](protocol-v1.md)。缓存介质、生产传输预算和实体设备意外断电仍待验证；正常运行下缓存不耗尽是设计目标，当前不承诺实际硬件容量与吞吐。
