...
def captured(tmp_path):
    ...
    spec = synthetic_spec("recording", width=64, height=48, fps=10, imu_hz=20, chunk_seconds=0.2)
    store.create(spec)
...
def selection(job, segment="task-1"):
    return DatasetRequest(
        name="test dataset", selections=[{"processing_id": job["id"], "segment_ids": [segment]}]
    )
...
def test_arrival_never_processes_and_batch_is_independent(captured):
    ...
    try:
        batch = processor.submit(
            BatchRequest(recording_ids=["incomplete", spec.id, "missing"], backend="synthetic")
        )
...
def test_review_snapshot_and_reprocess_are_independent(captured):
    ...
    assert (
        get_dataset(store, dataset["id"])["items"][0]["segment"]["description"]
        != "Revised description"
    )
    assert (
        effective_annotations(store, job["id"])["tasks"][0]["description"] == "Revised description"
    )
    save_review(
        store,
        job["id"],
        ReviewPatch(
            segment_id="task-1", expected_revision=1, decision="usable", note="second check"
        ),
    )
    assert (
        effective_annotations(store, job["id"])["tasks"][0]["description"] == "Revised description"
    ...
    with pytest.raises(StoreError, match="refresh"):
        save_review(
            store,
            job["id"],
            ReviewPatch(segment_id="task-1", expected_revision=0, decision="excluded"),
        )
    second = process(store)
    assert second["id"] != job["id"]
    assert effective_annotations(store, second["id"])["tasks"][0]["review"]["revision"] == 0
    assert (
        read_result(store, job["id"])["result"]["annotations"]["tasks"][0]["description"]
        != "Revised description"
    )
...
def test_review_does_not_fabricate_geometry(captured):
    ...
    save_review(
        store, job["id"], ReviewPatch(segment_id="task-1", expected_revision=0, decision="usable")
    )
    dataset = create_dataset(store, selection(job))
...
def test_excluded_child_and_invalid_bounds_are_rejected(captured):
    ...
    with pytest.raises(StoreError, match="Operation must be contained"):
        save_review(
            ...
            job["id"],
            ReviewPatch(
                segment_id="task-1", expected_revision=0, decision="usable", end_ns=500_000_000
            ),
        )
    save_review(
        store,
        job["id"],
        ReviewPatch(segment_id="operation-1", expected_revision=0, decision="excluded"),
    )
...
def test_egowam_zarr_has_nonzero_valid_poses(captured):
    ...
    group = zarr.open_group(
        str(store.root / "exports" / report["id"] / report["episodes"][0]["path"]), mode="r"
    )
    assert group["left.obs_ee_pose"].shape == (10, 7)
    assert group["obs_head_pose"][0, 3] == 1
    assert abs(group["left.obs_ee_pose"][0, 0]) > 0.1
...
def test_annotation_references_and_time_are_validated():
    with pytest.raises(ValueError, match="Unknown related"):
        AnnotationSet.model_validate(
            {
                "tasks": [{"id": "t", "start_ns": 0, "end_ns": 10, "description": "task"}],
                "operations": [
                    {
                        "id": "o",
                        "task_id": "t",
...
def test_invalid_hand_frame_splits_training_windows(captured, monkeypatch):
    ...
    assert (
        read_result(store, job["id"])["result"]["geometry"]["frames"][4]["hands"]["right"][
            "wrist_valid"
        ]
        is True
    )
