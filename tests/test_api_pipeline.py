...
def test_api_pipeline_review_export_and_access_boundaries(tmp_path):
    ...
    with TestClient(app) as client:
        ...
        assert (
            client.post(
                "/api/v1/recordings", content=b"{}", headers={"Content-Length": "invalid"}
            ).status_code
            == 400
        )
        assert (
            client.post(
                "/api/v1/recordings", content=b"{}", headers={"Content-Length": str(20 * 1024**2)}
            ).status_code
            == 413
        )
        ...
        for p in packets:
            assert (
                client.put(
                    f"/api/v1/recordings/{spec.id}/packets/{p.sequence}", json=p.model_dump()
                ).status_code
                == 200
            )
        ...
        assert (
            client.put(f"/api/v1/recordings/{spec.id}/end", json=end).json()["status"] == "complete"
        )
        jid = client.post(
            "/api/v1/processing", json={"recording_ids": [spec.id], "backend": "synthetic"}
        ).json()["jobs"][0]["id"]
        ...
        response = client.post(
            ...
            json={
                "name": "API pipeline",
                "selections": [{"processing_id": jid, "segment_ids": ["task-1"]}],
            },
        )
